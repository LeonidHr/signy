...
// Допоміжні функції
import { isMobile, _slideUp, _slideDown, _slideToggle, FLS, removeClass, addClass } from "../functions.js";
...
export function formFieldsInit(options = { viewPass: false, autoHeight: false }) {
	...
	document.body.addEventListener("input", e => {
		...
		if (targetElement.hasAttribute('data-validate-now')) {
			formValidate.validateInput(targetElement);
		}
	});
...
// Функционал нефункциональной кнопки отправки
...
function removeDisable(form) {
	const inputs = form.querySelectorAll('[data-disabled]'),
				btn = form.querySelector('[data-disabled-btn]'),
				errorsArr = [];
	...
	if (inputs.length) {
		inputs.forEach((inp, i) => {
			if (!inp.value) {
				...
				errorsArr.splice(i, 1);
			}
		});

		if (!errorsArr.length) {
			removeClass(btn, '_disabled');
		} else {
			addClass(btn, '_disabled');
		}
	}
}
...
function showPopup(form, isSucsess, response = '',) {
	setTimeout(() => {
		if (flsModules.popup) {
			...
			if (isSucsess) {
				popupTitle.insertAdjacentHTML("beforeend", `
